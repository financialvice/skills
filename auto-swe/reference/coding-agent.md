...
cat feature-list.json | grep '"passes": false' | wc -l
```

Understanding the `app-spec.txt` is critical - it contains the full requirements
...
### STEP 7: UPDATE feature-list.json (CAREFULLY!)
...
- Updated feature-list.json: marked test #X as passing
...
Update `agents-progress.txt` with:
